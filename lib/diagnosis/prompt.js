...
// DB(rulepack_versions.is_active)에서 활성 버전을 못 찾았을 때만 쓰는 fallback.
// 정상 운영 중에는 DB의 활성 버전이 우선이며, 이 파일은 로컬 개발/장애 대비용이다.
export const FALLBACK_RULEPACK = rulepackV1
...
// legal_sources는 룰팩 최신성 점검(관리자 도구)용 메타데이터(해시·MST 등)라 진단 판단과
// 무관하다. 매 진단 요청의 프롬프트에 그대로 실으면 토큰만 낭비하므로 제외하고 보낸다.
function stripMaintenanceMetadata(rulepack) {
  if (!rulepack?.rules) return rulepack
  return {
    ...rulepack,
    rules: rulepack.rules.map(({ legal_sources, ...rule }) => rule),
  }
}

export function buildUserMessage(profile, documents, rulepack) {
...
[진단 체크리스트 (rulepack v${rulepack?.meta?.version ?? '?'})]
${JSON.stringify(stripMaintenanceMetadata(rulepack))}
