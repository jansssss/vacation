...
const ConsultationCTA = ({ sourceSlug }) => {
  ...
  const inputBase = 'w-full rounded-xl bg-white/10 border border-white/15 px-4 py-3 text-sm text-white placeholder-slate-400 outline-none transition focus:border-blue-400 focus:ring-1 focus:ring-blue-400/50'
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-4 sm:gap-8 mb-10">
          {TRUST_ITEMS.map(({ icon, label, desc }) => (
            <div key={label} className="flex items-center gap-3 sm:flex-col sm:items-center sm:gap-2 sm:text-center">
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white/10 text-blue-300">{icon}</div>
              <div>
        ...
        <form onSubmit={handleSubmit} className="max-w-xl mx-auto space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-slate-400">이름 <span className="text-blue-400">*</span></label>
              <input type="text" name="name" value={form.name} onChange={handleChange} placeholder="홍길동" required className={inputBase} />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-slate-400">연락처 (전화 또는 이메일) <span className="text-blue-400">*</span></label>
              <input type="text" name="contact" value={form.contact} onChange={handleChange} placeholder="010-0000-0000" required className={inputBase} />
            </div>
          </div>

          <div className="space-y-1.5">
            <label className="text-xs font-medium text-slate-400">문의 유형</label>
            <select name="type" value={form.type} onChange={handleChange} className={`${inputBase} appearance-none`}>
          ...
          <div className="space-y-1.5">
            ...
            <textarea name="content" value={form.content} onChange={handleChange} placeholder="궁금하신 내용을 간략하게 적어주세요." rows={3} className={`${inputBase} resize-none`} />
          </div>

          <label className="flex items-start gap-3 cursor-pointer group">
            <div className="relative mt-0.5">
              <input type="checkbox" checked={agreed} onChange={(e) => setAgreed(e.target.checked)} className="sr-only" />
              <div className={`h-4 w-4 rounded border transition ${agreed ? 'bg-blue-500 border-blue-500' : 'bg-white/10 border-white/20'} flex items-center justify-center`}>
                {agreed && <svg width="10" height="10" viewBox="0 0 12 12" fill="none"><polyline points="2,6 5,9 10,3" stroke="white" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /></svg>}
              </div>
            </div>
          ...
          {errorMsg && <p className="text-xs text-red-400 bg-red-400/10 rounded-lg px-4 py-2">{errorMsg}</p>}

          <button type="submit" disabled={status === 'loading'} className="w-full rounded-full bg-blue-500 py-3.5 text-sm font-semibold text-white transition hover:bg-blue-400 active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed">
